...
print("En la siguiente tabla se muestran las medias calculadas para Peso sin alimentar en cada grupo. En el caso de que no haya una única moda, se muestran todos los valores separados por ;")
...
print("En la siguiente tabla se muestran la media y mediana calculadas para Sangre ingerida en cada grupo. En el caso de que no haya una única moda, se muestran todos los valores separados por ;")
...
print("En la siguiente tabla se muestran las medias calculadas para Cantidad de huevos puestos por cada grupo. En el caso de que no haya una única moda, se muestran todos los valores separados por ;")
...
#No se calculan percentiles porque al ser un n muy chico no tiene mucho sentido.
...
#existe un 95% de que ese intervalo contenga el valor real de esa variable en la población
...
n_psa = ttp.solve_power(effect_size=effect_size_psa, alpha=alpha, power=power, ratio=1.0, alternative='two-sided')
n_psi = ttp.solve_power(effect_size=effect_size_psi, alpha=alpha, power=power, ratio=1.0, alternative='two-sided')
n_hp = ttp.solve_power(effect_size=effect_size_hp, alpha=alpha, power=power, ratio=1.0, alternative='two-sided')
n_if = ttp.solve_power(effect_size=effect_size_if, alpha=alpha, power=power, ratio=1.0, alternative='two-sided')
...
#El IF no se distribuye normalmente en el grupo interferido, se rechaza H0 (p<0.05)
...
print("El resultado del test de Levene para Peso sin alimentar en ambos grupos es", hc_psa)
print("El resultado del test de Levene para Sangre ingerida en el grupo control es", hc_psi)
print("El resultado del test de Levene para Huevos puestos en el grupo interferido es", hc_hp)
print("El resultado del test de Levene para Índice de fecundidad en ambos grupos es", hc_if)

#Las varianzas son homogéneas en todos los casos, no se rechaza H0 (p>0.05)
#completar demás supuestos (escribir)

#aplicar t-test para comparar, haciendo salvedad para el caso de IF del grupo int. Se puede probar est. no par para ese caso

#aplicar t-test

resultado_ttest_if = st.ttest_ind(if_control["indice_de_fecundidad"], if_int["indice_de_fecundidad"], alternative='two-sided')
print("El valor p resultante para la prueba de T para peso sin alimentar es: ", resultado_ttest_if.pvalue)
print("El estadístico resultante para la prueba de T para peso sin alimentar es: ", resultado_ttest_if.statistic)
...
print("El valor p resultante para la prueba no paramétrica U de Umann-Whitney para peso sin alimentar es: ",resultado_mw_if.pvalue)
print("El estadístico resultante para la prueba no paramétrica U de Umann-Whitney para peso sin alimentar es: ",resultado_mw_if.statistic)
...
#en lugar de hacerse a mano, se podría usar groupby?

obs_if_contingencia = np.array([[1,3],[5,0]])
res_if_chi2 = st.chi2_contingency(obs_if_contingencia)
print("El valor p resultante para la prueba de Chi2 sobre los datos de IF (alto/bajo) es:", res_if_chi2.pvalue)
print("El estadístico resultante para la prueba de Chi2 sobre los datos de IF (alto/bajo) es:", res_if_chi2.statistic)
print("Los grados de libertad calculados para la prueba de Chi2 sobre los datos de IF (alto/bajo) son:", res_if_chi2.dof)
...
#print(df_psa_psi_final)

#graficar (2 opciones)
sns.lmplot(x="peso_sin_alimentar", y="peso_sangre_ingerida", data=df_psa_psi_final)
#plt.show()

plt.scatter(tabla["peso_sin_alimentar"], tabla["peso_sangre_ingerida"])
#plt.show()

psa_psi_coef_corr = st.pearsonr(tabla["peso_sin_alimentar"], tabla["peso_sangre_ingerida"])
print("El coeficiente de correlación entre Peso sin alimentar y Sangre ingerida es", psa_psi_coef_corr.statistic, "y el valor p asociado es de",psa_psi_coef_corr.pvalue)
...
#print(df_psi_hp_final)

#graficar (2 opciones)
sns.lmplot(x="peso_sangre_ingerida", y="huevos_puestos", data=df_psi_hp_final)
plt.show()

plt.scatter(tabla["peso_sangre_ingerida"], tabla["huevos_puestos"])
plt.show()

psi_hp_coef_corr = st.pearsonr(tabla["peso_sangre_ingerida"], tabla["huevos_puestos"])
print("El coeficiente de correlación entre Sangre ingerida y huevos puestos es", psi_hp_coef_corr.statistic, "y el valor p asociado es de",psi_hp_coef_corr.pvalue)



#redactar todo en un .md e interpretar resultados
